...
<template>
  <section class="min-h-screen bg-white p-4 font-sans w-full" id="depoimentos">
    <div class="max-w-6xl mx-auto">
      <div class="text-center mb-10">
        <h2 class="text-2xl md:text-4xl font-black text-black">
          O que dizem sobre a Jeri 4XP
        </h2>
      </div>

      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4 italic">
        <div 
          v-for="item in testimonials" 
          :key="item.id"
          class="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col">
          <div class="flex justify-between items-start mb-2">
            <span class="text-orange-600 text-4xl font-serif leading-none">“</span>
          ...
          <p class="text-slate-700 text-lg leading-relaxed mb-2">
            {{ item.content }}
          </p>
          ...
          <div class="flex items-center gap-4 pt-6 mt-auto">
            ...
            <div class="not-italic">
              <h4 class="font-extrabold text-black leading-tight">{{ item.name }}</h4>
              <div class="flex items-center text-slate-700 text-xs mt-1">
