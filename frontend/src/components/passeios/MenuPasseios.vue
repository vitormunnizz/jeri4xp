...
import jeri1 from "../../assets/jeri1.webp"
  ...
  <section class="min-h-screen w-full font-sans bg-white">
    ...
    <div class="relative w-full h-140 overflow-hidden">
      ...
      <div class="flex h-full">
        ...
        <swiper @swiper="setSwiper" :modules="modules" :slides-per-view="1" :loop="true"
          :autoplay="{ delay: 8000, disableOnInteraction: false }" class="meu-swiper">
          <swiper-slide>
            <img :src="jeri1" class="w-full h-full object-cover" />
          </swiper-slide>
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-4 z-10">
          <span class="text-orange-600 font-bold uppercase tracking-widest leading-none mb-4">
      ...
      <button @click="prev"
        class="absolute left-5 top-1/2 -translate-y-1/2 text-white text-5xl z-20 hover:cursor-pointer">
        ‹
      </button>

      <button @click="next"
        class="absolute right-5 top-1/2 -translate-y-1/2 text-white text-5xl z-20 hover:cursor-pointer">
        ›
      </button>
    </div>
    ...
    <div v-if="isMenuOpen"
      ...
      <button @click="isMenuOpen = false" class="absolute top-8 right-8 text-5xl hover:cursor-pointer">
        &times;
      </button>
